...
type Engine struct {
	...
	// When true, /list etc. only show sessions tracked by dirextalk-connect,
...
//
//	[dirextalk-connect sender_id=ou_abc123 platform=feishu]
//
...
func (e *Engine) getOrCreateWorkspaceAgent(workspace string) (Agent, *SessionManager, error) {
	...
	// already decorated. See dirextalk-connect#496 and the dirextalk-connect/core/runas.go
...
func (e *Engine) getOrCreateInteractiveStateWith(sessionKey string, p Platform, replyCtx any, session *Session, sessions *SessionManager, agentOverride Agent, ccSessionKey string) *interactiveState {
	...
	// Inject per-session env vars so the agent subprocess can call `dirextalk-connect cron add` etc.
	...
	// `/provider switch`; without restoring it here, a dirextalk-connect process
	...
	// (dirextalk-connect internal task t-20260614-qp7xnl).
	...
	// dirextalk-connect project that happens to share a Session row), the agent
...
func (e *Engine) handleCommand(p Platform, msg *Message, raw string) bool {
	...
	default:
		...
		// Not a dirextalk-connect command — notify user, then fall through to agent
...
// returned. When enabled, only sessions tracked by dirextalk-connect are shown.
...
// filterOwnedSessions removes agent sessions that are not tracked by dirextalk-connect's
...
func (e *Engine) switchProvider(p Platform, msg *Message, sessions *SessionManager, switcher ProviderSwitcher, name string) {
	...
	// dirextalk-connect process restart can re-bind the agent's activeIdx; without
	...
	// does not exist" errors against the wrong base_url. See dirextalk-connect
...
// It is used by the local API/CLI so agents can call `dirextalk-connect send --tts`.
...
// `dirextalk-connect send --audio`. Mirrors SendToSessionWithAttachments for
...
// back to FileSender otherwise. Used by `dirextalk-connect send --video`.
...
const ccConnectInstructionMarker = "<!-- dirextalk-connect-instructions -->"
...
func (e *Engine) buildSenderPrompt(content, userID, userName, platform, sessionKey, channelKey string) string {
	...
	if userName != "" {
		...
		return fmt.Sprintf("[dirextalk-connect sender_id=%s sender_name=\"%s\" platform=%s chat_id=%s]\n%s", userID, safeName, platform, chatID, content)
	}
	return fmt.Sprintf("[dirextalk-connect sender_id=%s platform=%s chat_id=%s]\n%s", userID, platform, chatID, content)
}
...
// dirextalk-connect process restart (i.e. the in-memory active provider is not
