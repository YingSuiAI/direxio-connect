...
func printAgentSIDUsage() {
	fmt.Println(`Usage: dirextalk-connect agent-sid [options]

Print the agent session ID (e.g. Claude Code, Codex, Gemini CLI) for the
current session. This is the ID used for --resume.

The command reads from the persisted session file; no running dirextalk-connect
...
Examples:
  dirextalk-connect agent-sid                         Auto-detect from env (inside a session)
  dirextalk-connect agent-sid -p dirextalk-agent-room -s "matrix:!room:example.com"`)
}
