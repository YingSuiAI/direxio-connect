...
// fetchLatestStableReleaseAsync asynchronously fetches the latest stable GitHub release.
...
func binaryAssetName(tag string) string {
	...
	name := fmt.Sprintf("%s-%s-%s-%s", binaryName, tag, goos, goarch)
	if goos == "windows" {
		name += ".exe"
	}
	return name
...
func archiveAssetName(tag string) string {
	goos := runtime.GOOS
	goarch := runtime.GOARCH
	base := fmt.Sprintf("%s-%s-%s-%s", binaryName, tag, goos, goarch)
	if goos == "windows" {
		return base + ".zip"
	}
	return base + ".tar.gz"
...
// (node_modules/dirextalk-connect/bin/) and updates the package.json version to
...
func syncNpmPackageVersion(execPath, newVer string) {
	...
	if err := os.WriteFile(pkgJSON, out, 0o644); err != nil {
		...
		fmt.Println("   please run: npm update -g dirextalk-connect")
	} else {
