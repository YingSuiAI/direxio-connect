...
// "at least one platform per project" check. Use this for management tooling
// that should work even before platforms are configured.
...
// requirement so management tooling can operate on agent-only configs before
...
func AddPlatformToProject(projectName string, platform PlatformConfig, workDir, agentType string) error {
	...
	if !strings.EqualFold(strings.TrimSpace(platform.Type), "matrix") {
		return fmt.Errorf("platform type must be \"matrix\"")
	}
